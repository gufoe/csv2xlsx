...
#[derive(StructOpt)]
#[structopt(version = "1.2", author = "Giacomo R. <gcmrzz@gmail.com>")]
struct Opts {
    #[structopt(short, long, help = "convert <input> to a csv file")]
    to_csv: bool,
    #[structopt(
        short,
        long,
...
fn excel_to_csv(opts: Opts) {
    let mut excel = open_workbook_auto(&opts.input).unwrap();
